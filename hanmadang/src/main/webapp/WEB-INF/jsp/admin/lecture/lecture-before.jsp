			...
			<div class="page-wrap">
				<div id="page-inner">
					...
					<article class="container-body">
						...
						<div class="h3-btn"><a href="${ADMIN_PATH}/entry/before/list" class="btn btn-level1 <c:if test="${empty mode or mode == 0 }">btn-default</c:if><c:if test="${mode == 1 }">btn-copy</c:if>" >수강응모 회원</a> 
						<a href="${ADMIN_PATH}/entry/before/list?mode=1" class="btn btn-level1 <c:if test="${!empty mode and mode == 1 }">btn-default</c:if><c:if test="${empty mode or mode == 0 }">btn-copy</c:if>" >선정/대기자</a></div>
						<div class="h3-desc">항목별로 지난 수강신청 회원을 검색할 수 있습니다.</div>
						<h3 class="h3-title">지난 데이타 검색</h3>
						
						<form id="class_search" method="post" action="${ADMIN_PATH}/entry/before/list">
					...
					<!-- 	<div class="h3-desc">
						...
						<div class="btn-group">
							<a href="${ADMIN_PATH}/entry/before/excel?mode=<c:out value="${mode }"/>&didx=<c:out value="${empty param.didx ? 0 : param.didx}"/>" class="btn btn-register btn-level2">엑셀로 다운로드 받기</a>
						</div>
