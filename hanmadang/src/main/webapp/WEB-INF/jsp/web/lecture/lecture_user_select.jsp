				...
				<div class="table-basic">
					<table class="table-basic-type2">
						...
						<tbody>
							...
							<c:choose>
							<c:when test="${item.b_cate == 19 && item.class_name != '종일' && lecture_session.mem_type != '본인'}"></c:when>
							<c:when test="${lecture_session.em_type == '01' && (item.b_cate == 19 or item.b_cate == 28 or item.b_cate == 26)  && lecture_session.mem_type != '본인' && lecture_session.mem_type != '배우자'}"></c:when>
							<c:when test="${lecture_session.em_type == '03' && item.class_c_count <= 0}"></c:when>
							<c:when test="${lecture_session.em_type2 == '01' && item.class_name == '오전' }"></c:when>
							<c:when test="${lecture_session.em_type2 == '01' && item.class_name == '오후' }"></c:when>
