...
<div id="wrapper">
			...
			<div class="modal-basic__wrapper modal-reserve__wrapper">
				...
				<div class="modal-basic__body">
					<div class="modal-basic__contents">
						...
						<div class="input-group">
							<select id="member-company" name="re_partname" class="text" style='
							font-size: 18px;
    color: #141414;
    text-align: center;
    line-height: 18px;
    ...
    padding: 4% 0;
							'>
								...
								<%@ include file="../common/cooperative-company.jsp"%>
							</select>
							<input type="text" name="re_tel" placeholder="연락처"
...
4. 동의 거부의 권리 및 불이익
위 개인정보 수집 이용에 대한 동의를 거부할 권리가 있습니다. 그러나 동의를 거부할 경우 대관 예약 및 체육관시설 이용에 일부 제한을 받을 수 있습니다.
							</textarea>
							<input type="radio" name="agree" id='agree' value='Y' style='opacity: 1;' checked/><label for='agree'>동의</label>
							<input type="radio" name="agree" id='agree2' value='N' style='opacity: 1;'/><label for='agree2'>미동의</label>
						</div>
						<div class="btn-group">
						...
						<div class="input-group">
							<p style="font: caption;font-size: 14px;margin: 14px 0px;">* 개인정보 입력 시 팝업창 또는 페이지 확인, 정보 입력/동의 必 수집항목, 목적, 보유기간, 동의거부 시 개인 예약 불가</p>
						</div>
						<style>.input-group{    margin: 0px auto;}</style>
					</div>
				</div>
				<a href="#none" title="닫기" class="modal-basic__btn-close js-modal__btn-close"><img src="/web/assets/img/btn-close.png" alt="레이어 닫기" /></a>
	...
	$('form').ajaxForm({
        beforeSubmit: function (data,form,option) {
        	...
        	if(type == 1){
	            ...
	            if(input_value == ''){
	            	alert("직번을 입력해주세요.");
	            	form.find('input[name="employee_no"]').focus();
	            	return false;
	            }
	            
	            input_value = form.find('input[name="re_tel"]').val();
	            if(input_value == ''){
	            	alert("연락처를 입력해주세요.");
	            	form.find('input[name="re_tel"]').focus();
	            	return false;
	            }
	            ...
	            input_value = form.find('input[name="agree"]:checked').val();
	            console.log(input_value);
	            if(input_value=='' || input_value == 'N'){
	            	alert("개인정보의 처리방침에 동의 하셔야합니다.");
	            	return false;
	            }
